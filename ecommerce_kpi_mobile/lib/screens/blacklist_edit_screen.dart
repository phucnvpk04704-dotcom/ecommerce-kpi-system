...
class BlacklistEditScreen extends StatefulWidget {
  final String customerId;
  final BlacklistService? service;

  const BlacklistEditScreen({
    super.key,
  ...
  @override
  State<BlacklistEditScreen> createState() => _BlacklistEditScreenState();
}

class _BlacklistEditScreenState extends State<BlacklistEditScreen> {
  final _formKey = GlobalKey<FormState>();
  final _nameController = TextEditingController();
  final _phoneController = TextEditingController();
  final _cancelCountController = TextEditingController();
  final _returnCountController = TextEditingController();
  final _complaintCountController = TextEditingController();
  ...
  void _initFields(dynamic cust) {
    if (_dataLoaded) return;
    _nameController.text = cust.customerName;
    _phoneController.text = cust.phone;
    _cancelCountController.text = cust.cancelCount.toString();
    _returnCountController.text = cust.returnCount.toString();
    _complaintCountController.text = cust.complaintCount.toString();
    _noteController.text = cust.note;
    _selectedRisk = cust.riskLevel;
    _selectedPlatform = cust.platform;
    _dataLoaded = true;
  }
                              ...
                              children: [
                                ...
                                provider.loading
                                    ...
                                    : ElevatedButton(
                                        ...
                                        onPressed: () => _submitForm(context, provider, cust!),
                                        child: const Text(
  ...
  void _submitForm(BuildContext context, BlacklistProvider provider, dynamic cust) async {
    if (_formKey.currentState!.validate()) {
      final data = {
        ...
        'last_violation_date': cust.lastViolationDate,
      };

      final success = await provider.updateCustomer(cust.id, data);
      if (context.mounted) {
        if (success) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Customer profile updated successfully!'), backgroundColor: Colors.green),
          );
          context.go('/blacklist_detail_screen/${cust.id}');
        } else {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text(provider.error ?? 'Failed to update customer profile.'), backgroundColor: Colors.red),
          );
        }
      }
    }
